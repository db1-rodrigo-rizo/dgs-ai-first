...
export function validateResponse(raw: unknown): ValidatedResponse {
	...
	const hasDangerousCargoReference = /\b(cargas?|materiais?|produtos?|mercadorias?)\s+perigos[ao]s?\b/.test(normalizedAnswer);
	const hasReturnReference = /\bdevol(ucao|ver[ao]?|vida|vido|vendo|va|veu|veram|vemos)\b/.test(normalizedAnswer);
	const hasExplicitNegative = /(\bnao\b|\bimpossivel\b|\bvedado\b|\bproibido\b)/.test(normalizedAnswer);
